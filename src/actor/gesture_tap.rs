...
impl GestureTap {
    ...
    fn handle_scroll(&self, handler: &ScrollHandler, touches: ScrollTouchFrame) -> bool {
        ...
        if state.accum_dx.abs() >= cfg.distance_pct {
            // `speed` scales the committed step only. `invert_horizontal` is already
            // applied to `dx` above, before accumulation, so the multiplier must not
            // re-apply it: the pre-rewrite code inverted at this emission site, and
            // transcribing that verbatim onto upstream's new order double-inverts.
            let delta = cfg.speed * state.accum_dx;
            state.accum_dx = 0.0;
