            ...
            LayoutEvent::WindowsOnScreenUpdated(space, pid, windows_with_titles, app_info) => {
                ...
                if let Some((window, workspace)) = focus_request {
                    ...
                    return EventResponse {
                        ...
                        boundary_hit: None,
                        // An app rule with `focus = true` is a deliberate focus
                        // grant, so it follows the same policy as every other
                        // focus path (§2.B `[settings.layout] activate_on_focus`).
                        activate: self.layout_settings.activate_on_focus,
                    };
                }
